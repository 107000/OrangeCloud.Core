...
namespace OrangeCloud.Core
{
    public static class MySqlMapping
    {
        ...
        public static string SqlServerGetList<T>(string tableName, string where, string orderBy, int pageIndex, out string sqlCount, int pageSize, string showCols = "*", ESqlVersion sqlVersion = ESqlVersion.SqlServer2012, bool nolock = true)
        {
            ...
            return ComPublicStatic.Save(GetMyServerPagedSql(sbSql.ToString(), pageIndex, pageSize, orderBy));
        }
        ...
        /// <returns></returns>
        //public static string GetMyServerPagedSqlWith2005(string selectSql, int pageIndex, int pageSize, string orderBySql = null)
        //{
        ...
        /// <param name="orderBySql">用于排序的sql语句，例如 t1.id desc</param>
        /// <returns></returns>
        public static string GetMyServerPagedSql(string selectSql, int pageIndex, int pageSize, string orderBySql = null)
        {
            string select = "SELECT";
            int sIndex = selectSql.IndexOf(select, 0, StringComparison.CurrentCultureIgnoreCase);
            if (sIndex != -1) selectSql = " " + selectSql.Substring(sIndex + select.Length);
            ...
            if (orderBySql == null)
                orderBySql = "(SELECT 0)";
            string strSql = string.Format("SELECT {1} {0} limit {2},{3} ",
